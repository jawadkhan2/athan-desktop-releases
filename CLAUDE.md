...
- Distribution: GitHub Releases on the **public** `jawadkhan2/athan-desktop-releases` repo. Code and releases share this one public repo — the workflow publishes to its own repo, so the auto-provisioned `GITHUB_TOKEN` suffices (no cross-repo PAT, nothing to expire). Users install once via the NSIS installer (`Athan_X.Y.Z_x64-setup.exe`, per-user, no UAC); after that the app self-updates via `tauri-plugin-updater` against `releases/latest/download/latest.json`.
- The old private `jawadkhan2/athan-desktop` repo is archived (read-only backup of pre-consolidation history). The updater endpoint URL is baked into shipped binaries, so the canonical repo must keep the exact name `athan-desktop-releases`.
